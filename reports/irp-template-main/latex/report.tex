...
manually altered. The current set contains 16 videos:

\begin{itemize}
    \item four correct single-pendulum videos;
    \item four wrong single-pendulum videos;
    \item four correct double-pendulum videos;
    \item four wrong double-pendulum videos.
\end{itemize}

The wrong-physics interventions include energy gain, gravity reversal after the
middle of the video, and periodic velocity kicks. Unlike the AI-generated
videos, these labels are controlled by construction. This makes the simulated
dataset a better benchmark for testing whether the evaluation pipeline is
stable and whether latent trajectories respond to physical inconsistency.
...
The simulated pendulum benchmark contains 16 videos and was evaluated with nine
encoders, producing 144 successful evaluation rows. Tables
\ref{tab:sim-encoder-deltas}--\ref{tab:sim-scene-deltas} summarize the main
quantitative results.

\input{generated/simulated_pendulum/tables}
...
\input{generated/simulated_pendulum/figures}
...
CLIP-B/32 has the highest AUC for mean step distance in this benchmark
($0.844$), outperforming CLIP-L/14 on this specific metric. This does not imply
that the smaller CLIP model is generally better; it suggests that patch size,
embedding scale, and training objective can affect sensitivity to simple
geometric motion. SigLIP, Swin, and ViT also show positive step-distance deltas,
but the separation is smaller. MAE-B does not provide a useful signal in this
setting.
...
DINOv2 gives the largest absolute step-distance changes, while CLIP-B/32 gives
the strongest AUC for mean step distance. MAE performs poorly, suggesting that
not all pretrained image encoders are equally useful for temporal physical
diagnostics. A single universal score would hide these differences, so the
report should present model-specific behaviour.
...
The next step is to strengthen the qualitative analysis. Patch-level feature
change maps and attention overlays can show whether the encoder response is
located on the moving pendulum components. If the largest feature changes occur
around the bob, rod, or pivot during wrong-physics interventions, that would
make the trajectory metrics more interpretable. If the maps focus on unrelated
regions, the scalar metrics would be less physically meaningful.
